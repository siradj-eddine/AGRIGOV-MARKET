    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
      <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-sm p-7 space-y-4">
        ...
        <p className="text-sm text-slate-600 dark:text-slate-300 bg-slate-50 dark:bg-slate-800 rounded-xl p-3">
          ...
          <span className="font-bold">{name}</span>? All images and listings will be permanently removed.
        </p>
...
export default function ProductManagementPage() {
  ...
  const handleToggleStock = useCallback(async (id: number, current: boolean) => {
    setProducts((prev) =>
  ...
  const stats = useMemo(() => {
    ...
    const avgRating =
      rated.length > 0
        ? (rated.reduce((s, p) => s + p.average_rating, 0) / rated.length).toFixed(1)
        : "—";
      ...
      <div className="space-y-6 pb-24 lg:pb-6">
        ...
        <div className="flex flex-col sm:flex-row gap-3 flex-wrap">
          ...
          <button
            ...
            onClick={() => {
              setFilters((f) => ({ ...f, in_stock: f.in_stock === true ? null : true }));
              resetPage();
            }}
